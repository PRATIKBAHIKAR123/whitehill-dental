...
const StepAppointment = ({
  ...
  useEffect(() => {
    ...
    const loadBookedSlots = async () => {
      try {
        ...
        // Create date strings from local date components to avoid timezone shift
        const fromDateStr = `${fromDateVisible.getFullYear()}-${String(
          fromDateVisible.getMonth() + 1
        ).padStart(2, "0")}-${String(fromDateVisible.getDate()).padStart(
          2,
          "0"
        )}`;
        const toDateStr = `${toDateVisible.getFullYear()}-${String(
          toDateVisible.getMonth() + 1
        ).padStart(2, "0")}-${String(toDateVisible.getDate()).padStart(
          2,
          "0"
        )}`;

        const fromDate = fromDateStr + "T00:00:00.00";
        const toDate = toDateStr + "T23:59:59.99";

        // Check if we already have cached data for this week and location
        const cacheKey = `${selectedLocation}-${fromDateStr}`;
  ...
  const generateDates = useCallback(() => {
    ...
    for (let i = 0; i < 84; i++) {
      ...
      // Create ISO date string from local date components to avoid timezone shift
  ...
  const getTimeSlotsForDate = async (dateObj: any, locationId: number) => {
    ...
    try {
      ...
      // Parse date in local timezone to avoid timezone shift issues
      const [year, month, day] = dateObj.fullDate.split("-").map(Number);
      ...
      // Create date string from local date components to avoid timezone shift
      ...
      const cacheKey = `${locationId}-${weekStartStr}`;
      ...
      const timeSlotsWithAvailability = allTimeSlots.map((timeSlot) => {
        // Create today's date string from local date components to avoid timezone shift
        const today = new Date();
          ...
          <div className="relative">
            ...
            <div
              ...
              className={`w-5 h-5 rounded border-2 cursor-pointer flex items-center justify-center ${
                isNewClient ? "bg-teal-600 border-teal-600" : "border-gray-300"
              }`}
            >
