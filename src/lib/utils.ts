...
export function generateGoogleCalendarUrl(appointmentData: {
  ...
  if (appointmentData.appointmentDate) {
    ...
    dateOnly = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(
      2,
      "0"
    )}-${String(date.getDate()).padStart(2, "0")}`;
  } else {
    // Fallback to current date using local components
    const date = new Date();
    dateOnly = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(
      2,
      "0"
  ...
  if (appointmentData.startTime && appointmentData.endTime) {
    ...
    if (appointmentData.startTime.includes(":")) {
      if (
        appointmentData.startTime.includes("AM") ||
        appointmentData.startTime.includes("PM")
      ) {
        ...
        [startHours, startMinutes] = appointmentData.startTime
          .split(":")
          .map(Number);
      }
    } else {
    ...
    if (appointmentData.endTime.includes(":")) {
      if (
        appointmentData.endTime.includes("AM") ||
        appointmentData.endTime.includes("PM")
      ) {
  ...
  const eventTitle = encodeURIComponent(
    `Dental Appointment - ${appointmentData.patientName}`
  );
  const eventDetails = encodeURIComponent(
    `Patient: ${appointmentData.patientName}\n` +
      ...
      `Location: ${
        appointmentData.locationName || appointmentData.location
      }\n` +
      (appointmentData.address &&
      appointmentData.city &&
      appointmentData.state &&
      ...
      (appointmentData.hasInsurance && appointmentData.insuranceName
        ? `Insurance: ${appointmentData.insuranceName}\n`
        : "") +
  ...
  const calendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${eventTitle}&dates=${startDateTime}/${endDateTime}&details=${eventDetails}&location=${encodeURIComponent(
    locationString
  )}&sf=true&output=xml`;
