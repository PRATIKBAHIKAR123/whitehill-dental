  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setStatusMessage(result.message || 'Message sent successfully! We\'ll contact you shortly.');
    ...
    <div className="fixed bottom-6 right-6 z-50 pb-[env(safe-area-inset-bottom)]">
      <Dialog open={open} onOpenChange={(newOpen) => {
        ...
        <DialogTrigger asChild>
          <button
            className="w-14 h-14 bg-primary hover:bg-blue-700 text-white rounded-full flex items-center justify-center shadow-lg transition-all duration-300"
            aria-label="Text us"
          >
        ...
        <DialogContent className="max-w-md !px-0 !py-0 [&>button]:hidden">
          <DialogHeader className="bg-primary text-white px-6 py-4 rounded-t-lg">
            ...
            <p className="text-sm mt-1">Enter your info and we'll call you shortly.</p>
            <DialogClose
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            {/* Status Message */}
            {submitStatus !== 'idle' && (
              <div className={`p-3 rounded-lg flex items-center gap-2 ${submitStatus === 'success'
            ...
            <p className="text-xs text-gray-500 mt-2">
              ...
              <a href="/terms-and-conditions" className="underline">Terms of Use</a>, and I consent to receiving text messages from this practice acknowledging that data rates may apply. Message frequency varies.
            </p>
